...
\begin{figure}[t]
    ...
    \caption{\textbf{Packed WDM coefficient layout.} The transform size is fixed by $N=N_tN_f$, but the stored channel index runs from $m=0$ to $m=N_f$. The two endpoint channels correspond to the DC and Nyquist edge channels, so the packed coefficient array has shape $N_t \times (N_f+1)$.}
