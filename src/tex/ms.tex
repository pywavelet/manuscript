...
To illustrate the utility of \package\ on astrophysically relevant signals, we apply the WDM transform to three synthetic gravitational‐wave signals representative of key LISA source classes: (i) an \emph{extreme mass‐ratio inspiral} (EMRI)‐like waveform with a slow power‐law frequency evolution, (ii) a superposition of \emph{verification galactic binary} (VGB)‐like quasi‐monochromatic signals at distinct frequencies, and (iii) a \emph{massive black hole binary} (MBHB)‐like signal with a rapid inspiral chirp and ringdown.
Figure~\ref{fig:wdm_real_waveforms} shows the WDM-domain amplitude maps for each signal. Each panel displays the packed WDM coefficient magnitude $|W_{nm}|$ as a function of time bin $n$ and frequency channel $m$.

\begin{figure*}[h]
    \centering
    \includegraphics[width=1\linewidth]{figures/example_transforms.pdf}
    \caption{WDM‐domain representations of three synthetic gravitational‐wave signals representative of LISA source classes: (a) EMRI‐like slowly chirping waveform; (b) VGB‐like superposition of quasi‐monochromatic binaries; (c) MBHB‐like rapid chirp with inspiral and ringdown. Each panel shows the packed WDM coefficient magnitude $|W_{nm}|$.}
